...
dotnet build -v n interpc.fsproj # 构建./bin/Debug/net8.0/interpc.exe ，-v n查看详细生成过程

# 执行解释器
#./bin/Debug/net8.0/interpc.exe example/ex1.c 8
...
dotnet build  microc.fsproj   # 构建 ./bin/Debug/net8.0/microc.exe
...
#./bin/Debug/net8.0/microc.exe -g example/ex1.c  # 直接执行构建的.exe文件，同上效果
...
- 运行编译器生成的 *.out  文件 需要先完成 D.1(c#版本)或 D.2(c版本) ，在当前目录生成虚拟机`machine.exe`
...
./bin/Debug/net8.0/microcc.exe ex11.c  # 直接执行
...
./bin/Debug/net8.0/machine.exe ./example/ex9.out 3  # 运行虚拟机，执行 ex9.out 
./bin/Debug/net8.0/machine.exe -t ./example/ex9.out 0  # 运行虚拟机，执行 ex9.out ，-t 查看跟踪信息
./bin/Debug/net8.0/machine.exe -t ./example/ex9.out 3  // 运行虚拟机，执行 ex9.out ，-t 查看跟踪信息
```
